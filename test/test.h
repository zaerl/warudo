...
#define ASSERT_STRING_INTERNAL(MESSAGE, TEST, RES, ...) warudo_assert((strcmp(TEST(__VA_ARGS__), RES)) == 0, 0, #TEST, MESSAGE);

#define ASSERT(MESSAGE, RES, TEST, ...) ASSERT_INTERNAL(MESSAGE, TEST, RES, __VA_ARGS__)
#define ASSERT_STRING(MESSAGE, RES, TEST, ...) ASSERT_STRING_INTERNAL(MESSAGE, TEST, RES, __VA_ARGS__)
